/**
 * The self-hosted Lucide subset.
 *
 * DESIGN.md → Components names `claude/mockups/assets/lucide-icons.js` as the icon set
 * and lists the glyphs this product adds to it: pause, play, user, user-check, braces,
 * cpu, help-circle, shield, bell, flag, archive, search-x. The ones that asset already
 * carries are copied from it character for character; the ones it does not are the
 * upstream Lucide paths for those names, at the same 24x24 grid and stroke geometry.
 *
 * Icons ship with the application. There is no CDN, no icon package, and no runtime
 * fetch: a deployment with no outbound network still renders every status.
 *
 * Glyph paths are from Lucide (https://lucide.dev), ISC licensed.
 */

/** The inner markup of each 24x24 `viewBox` glyph. */
export const ICON_GLYPHS = {
  // --- present in the self-hosted asset -------------------------------------
  'alert-circle':
...
export function isIconName(value: unknown): value is IconName {
  return typeof value === 'string' && Object.hasOwn(ICON_GLYPHS, value);
}
