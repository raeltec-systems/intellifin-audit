        ...
        return <fieldset className="ls-stack" key={condition.conditionId} data-condition-id={condition.conditionId} onBlur={() => touch(condition.conditionId)}>
          ...
          {availableComparison === null ? null : <div className="ls-stack">
            {condition.comparison === null ? null : <>
              ...
              <p className="ls-caption" id={`${fieldId}-comparison-help`}>Type exact numbers. The allowed difference is how far past the limit still counts as acceptable; use 0 for none. {isWindow ? 'The threshold and tolerance are in hours.' : draft.templateId === 'P-3' && compiled?.rule?.kind === 'approval' ? 'The threshold and tolerance are in USD; tolerance also allows that difference in the approver limit.' : 'The threshold and tolerance use the Observation field’s units.'} Changing these changes what this rule counts as a finding.</p>
            </>}
          </div>}
