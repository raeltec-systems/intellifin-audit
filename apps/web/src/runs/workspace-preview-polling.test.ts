...
describe('sequential preview polling cadence', () => {
  it('includes fetch/decode/check work inside each half second instead of adding it to the interval', async () => {
    vi.useFakeTimers();
    const starts: number[] = [];
    let active = 0, peak = 0;
    ...
    stop();
    expect(starts.map(value => value - starts[0]!)).toEqual([0, 500, 1000]);
    expect(peak).toBe(1);
  });
