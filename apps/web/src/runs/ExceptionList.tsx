  ...
  return (
    <li className="ls-exception ls-stack" id={`exception-${exception.exceptionId}`}>
      ...
      {/* The two one-click destinations EXPERIENCE.md asks a finding to carry: the record's
          own evidence, and the session frame where it was captured. The Replay hash is the
          Work Item jump target `ReplayViewer` selects on load; the Evidence anchor is the
          grounding inspector's own `id`. */}
      <p className="ls-exception__links">
        <a href={`/runs/${runId}/evidence#observation-${exception.observationId}`}>
          {EXCEPTION_WORDS.openEvidence}
          <span className="ls-visually-hidden"> for {record}</span>
        </a>
        {' · '}
        <a href={`/runs/${runId}/replay#work-item-${exception.workItemId}`}>
