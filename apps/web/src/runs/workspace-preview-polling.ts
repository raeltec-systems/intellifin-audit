/** Poll starts are half a second apart while work fits inside that interval. Network,
...
export function startWorkspacePreviewPolling(poll: () => Promise<void>): () => void {
  ...
  const run = () => {
    ...
    const settled = () => {
      if (!stopped) timer = setTimeout(run, Math.max(0, 500 - (performance.now() - started)));
    };
