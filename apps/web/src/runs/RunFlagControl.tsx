...
export function RunFlagControl({ runId, flaggable, flags }: RunFlagControlProps): React.JSX.Element {
  ...
  return <details id="run-flag" className="ls-flag-menu" open={state !== null ? true : undefined}>
    ...
    <section className="ls-card ls-stack ls-flag-menu__panel" aria-labelledby="run-flag-heading">
      ...
      {flaggable ? (
        // The form keeps its explicit native POST and the React action rides on the
        // submitter (`Button.formAction`): a function action on the form itself conflicts
        // with an explicit method, and the no-JavaScript submission must stay a POST.
