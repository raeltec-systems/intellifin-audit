...
const mocks = vi.hoisted(() => ({ authorize: vi.fn(), runtime: vi.fn(), acquire: vi.fn(), renew: vi.fn(), release: vi.fn(), propose: vi.fn(), confirm: vi.fn(), recover: vi.fn(), read: vi.fn() }));
...
describe('distinct manager transfer server boundaries', () => {
  it.each([proposeRunControlTransferAction, confirmRunControlTransferAction])('refuses before reading untrusted input', async action => {
    mocks.authorize.mockResolvedValue({ allowed: false, reason: 'Access denied.' });
    const hostile = new Proxy({}, { get: () => { throw new Error('read'); }, ownKeys: () => { throw new Error('enumerated'); } });
    expect(await action(hostile, session.userId)).toEqual({ ok: false, code: 'unauthorized', reason: 'Access denied.' });
    expect(mocks.runtime).not.toHaveBeenCalled();
