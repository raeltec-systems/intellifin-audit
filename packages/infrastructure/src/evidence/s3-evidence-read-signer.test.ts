...
describe('S3EvidenceReadSigner', () => {
  it('delegates exact GET signing to the injected standard SDK presigner', async () => {
    const calls: { bucket?: string; key?: string; responseContentType?: string; expiresIn?: number } = {};
    const presigner: S3GetUrlPresigner = {
      getSignedUrl: async (_client, command, options) => {
        ...
        calls.responseContentType = command.input.ResponseContentType;
    ...
    expect(calls).toEqual({ bucket: 'evidence', key: 'runs/run/evidence/snapshot', responseContentType: 'image/png', expiresIn: 300 });
  });
  ...
  it('refuses expiry outside the five minute bound', async () => {
    const presigner: S3GetUrlPresigner = { getSignedUrl: async () => 'https://objects.invalid' };
    const signer = new S3EvidenceReadSigner({ bucket: 'evidence', client: {} as S3Client, presigner, now: () => NOW });
    await expect(signer.signGet({ bucketKey: 'runs/run/object', expiresAt: '2026-09-07T00:05:01.000Z', responseMediaType: 'image/png' })).rejects.toThrow('outside the bound');
    await expect(signer.signGet({ bucketKey: 'runs/run/object', expiresAt: '2026-09-06T23:59:59.000Z', responseMediaType: 'image/png' })).rejects.toThrow('expired');
  });
});
