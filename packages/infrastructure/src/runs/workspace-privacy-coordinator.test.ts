...
describe('same-workspace privacy and preview coordinator', () => {
  ...
  it.each([500, 1000])('coalesces viewers into one capture slot and enforces the %i ms interval', async intervalMs => {
    const { coordinator, admit, advance } = fixture({ intervalMs });
    const fence = admit();
  ...
  it.each([
    { intervalMs: 499 }, { maxFrameBytes: 0 }, { maxFrameBytes: 8 * 1024 * 1024 + 1 },
