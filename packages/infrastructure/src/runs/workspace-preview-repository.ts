 ...
 * workspace_revision names the first claim of a workspace incarnation, not the
 * mutable provisioning CAS revision. Reattachment keeps the local workspace ID. */
export class PostgresWorkspacePreviewStore implements WorkspacePreviewMetadataStore {
  ...
  async claim(ref: WorkspaceRef, runtimeId: string): Promise<number | null> {
    ...
    const rows = await this.db.execute<{ workspace_revision: number }>(sql`
      INSERT INTO run_workspace_preview(run_id,workspace_id,workspace_revision,runtime_id,privacy_epoch,mode,sequence,expires_at)
      SELECT w.run_id,w.workspace_id,w.revision,${runtimeId}::uuid,0,'unavailable',0,clock_timestamp()+interval '4 seconds'
      ...
      ON CONFLICT(run_id) DO UPDATE SET workspace_id=excluded.workspace_id,workspace_revision=excluded.workspace_revision,runtime_id=excluded.runtime_id,
      ...
      WHERE run_workspace_preview.workspace_id<>excluded.workspace_id AND run_workspace_preview.workspace_revision<excluded.workspace_revision
  ...
  async publish(value: WorkspacePreviewMetadata): Promise<boolean> {
    const rows = await this.db.execute(sql`
      ...
      FROM run_workspace w,audit_run r WHERE p.run_id=${value.runId}::uuid AND p.runtime_id=${value.runtimeId}::uuid
        ...
        AND w.run_id=p.run_id AND w.workspace_id=p.workspace_id AND w.mode='local' AND w.status IN ('OPEN','PROVISIONING') AND r.run_id=p.run_id
  ...
  async current(value: WorkspacePreviewIdentity): Promise<boolean> {
    const rows = await this.db.execute(sql`SELECT p.run_id FROM run_workspace_preview p
      ...
      WHERE p.run_id=${value.runId}::uuid AND p.runtime_id=${value.runtimeId}::uuid AND p.workspace_revision=${value.workspaceRevision}
        ...
        AND w.workspace_id=p.workspace_id AND w.mode='local' AND w.status IN ('OPEN','PROVISIONING') AND r.state IN ('RUNNING','PAUSED','AWAITING_AUDITOR')`);
    return rows.length === 1;
  }
  async authorized(runId: string, viewer: WorkspacePreviewViewer): Promise<WorkspacePreviewMetadata | null> {
    ...
    const rows = await this.db.execute<WorkspacePreviewMetadata & { role: string; [key: string]: unknown }>(sql`
      ...
      WHERE p.run_id=${runId}::uuid AND p.expires_at>clock_timestamp() AND w.workspace_id=p.workspace_id AND w.mode='local' AND w.status='OPEN'
        AND r.state IN ('RUNNING','PAUSED','AWAITING_AUDITOR')`);
    const row = rows[0];
