...
ALTER TABLE "audit_run" ADD COLUMN "cancel_requested_command_id" uuid;--> statement-breakpoint
ALTER TABLE "audit_run" ADD CONSTRAINT "audit_run_cancel_command" CHECK ("audit_run"."cancel_requested_command_id" IS NULL OR "audit_run"."cancel_requested_at" IS NOT NULL);--> statement-breakpoint
...
BEGIN
  ...
  IF NEW.sequence<>coalesce(prior.sequence,0)+1 OR NOT coalesce((
    ...
    (prior.state='interpreted' AND (NEW.state IN ('queued','refused') OR (cmd.kind IN ('resume','stop') AND NEW.state='applied'))) OR (prior.state='queued' AND NEW.state IN ('applied','superseded'))
...
--> statement-breakpoint
-- The accepted marker is a durable latch, never a mutable pointer to another command.
CREATE FUNCTION run_cancel_interaction_guard() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE cmd run_interaction_command%ROWTYPE;
BEGIN
  IF TG_OP='UPDATE' AND OLD.cancel_requested_at IS NOT NULL THEN
    IF OLD.cancel_requested_command_id IS DISTINCT FROM NEW.cancel_requested_command_id OR
      (OLD.cancel_requested_command_id IS NOT NULL AND
        ROW(OLD.cancel_requested_at,OLD.cancel_requested_by,OLD.cancel_requested_session,OLD.cancel_reason)
        IS DISTINCT FROM ROW(NEW.cancel_requested_at,NEW.cancel_requested_by,NEW.cancel_requested_session,NEW.cancel_reason)) THEN
      RAISE EXCEPTION 'Accepted cancellation identity is immutable' USING ERRCODE='23514';
    END IF;
  END IF;
  IF NEW.cancel_requested_command_id IS NOT NULL THEN
    SELECT * INTO cmd FROM run_interaction_command WHERE command_id=NEW.cancel_requested_command_id;
    IF NOT FOUND OR cmd.kind<>'stop' OR cmd.run_id<>NEW.run_id OR cmd.actor_id IS DISTINCT FROM NEW.cancel_requested_by THEN
      RAISE EXCEPTION 'Cancellation marker must bind its Stop command, Run and actor' USING ERRCODE='23514';
    END IF;
    IF TG_OP='INSERT' OR (OLD.cancel_requested_at IS NULL AND cmd.expected_run_revision<>OLD.revision) THEN
      RAISE EXCEPTION 'Cancellation marker must bind its reviewed revision' USING ERRCODE='23514';
    END IF;
  END IF;
  RETURN NEW;
END $$;
--> statement-breakpoint
CREATE TRIGGER run_cancel_interaction_guard BEFORE INSERT OR UPDATE ON audit_run
FOR EACH ROW EXECUTE FUNCTION run_cancel_interaction_guard();
--> statement-breakpoint
-- The cancellation event precedes sealing inside performCancellation. Verify final
-- consistency at commit, including a later Run mutation in the same transaction.
CREATE FUNCTION run_stop_applied_consistency() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE target_run uuid;
BEGIN
  IF TG_TABLE_NAME='audit_run' THEN target_run:=NEW.run_id;
  ELSE SELECT run_id INTO target_run FROM run_interaction_command WHERE command_id=NEW.command_id AND kind='stop'; END IF;
  IF EXISTS (SELECT 1 FROM run_interaction_command c JOIN run_interaction_transition t ON t.command_id=c.command_id
    WHERE c.run_id=target_run AND c.kind='stop' AND t.state='applied') AND NOT EXISTS (
      SELECT 1 FROM audit_run r JOIN run_result rr ON rr.run_id=r.run_id JOIN run_evidence_package ep ON ep.run_id=r.run_id
      WHERE r.run_id=target_run AND r.state='CANCELED' AND rr.run_state='CANCELED' AND rr.outcome='CANCELED' AND rr.sealed AND ep.run_state='CANCELED'
    ) THEN RAISE EXCEPTION 'Applied Stop requires its actual sealed cancellation outcome' USING ERRCODE='23514'; END IF;
  RETURN NEW;
END $$;
--> statement-breakpoint
CREATE CONSTRAINT TRIGGER run_stop_applied_consistency AFTER INSERT ON run_interaction_transition
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION run_stop_applied_consistency();
--> statement-breakpoint
CREATE CONSTRAINT TRIGGER run_stop_applied_consistency AFTER UPDATE ON audit_run
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION run_stop_applied_consistency();
