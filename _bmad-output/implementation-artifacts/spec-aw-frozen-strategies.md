...
**Ask First:** D3 for manager transfer; this slice uses the existing holder permission only.
...
- [ ] Add version-2 canonical graph, historical compatibility and visible approval meaning.
- [ ] Derive eligibility from committed same-unit evidence and enforce it in tool planning.
- [ ] Persist and confirm exact selections with actor, controller, graph and work-unit binding.
- [ ] Consume at the existing worker boundary with durable action-linked receipts and recovery.
- [ ] Render truthful conversation interpretation, consequence and queued/applied/refused states.
