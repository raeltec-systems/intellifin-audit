...
## Current checkpoint — credential regression awaiting hosted execution
...
- Pushed runtime candidate `2d1eec4fe8799e49996a221b5fa8e43df0fc9d3c` passes hosted CI34196546462 PostgreSQL18:472tests/39files, fresh migrations, real sealed generation31 and populated generation40 upgrades through final41, repeat migration, drift, and all22selected unit/browser/PostgreSQL guard mutations. Static/type/unit/boundary and all three image build/startup jobs pass. CI checks merge SHA `67a4b1d6ef0fd9280bb441a0b102ed6a469e655f`; its full tree `96ac0d71c285a9a6a933a509d99ab54ddb8ac15b` equals that candidate's tree.
- That candidate's full local-browser/accessibility and separate actual-worker/hydrated mutation jobs are still running. Earlier notification/grounding/source/cleanup failures have coherent pushed repairs, but final browser acceptance remains pending. This checkpoint adds one actual-worker credential regression and its seventh worker/hydrated mutation; it is not yet hosted-tested.
...
- Stories4.4–4.11 have implementation checkpoints but remain in final verification. Additional compiled-worker P-1 review/absence and P-4 baseline journeys use explicitly synthetic provider HTTP with real worker, browser, PostgreSQL and evidence storage. They await final hosted results.
...
- Next: push the credential regression, complete every exact-candidate job and repair observed failures; retain per-case mutation results; execute the live gate when runner settings and target prerequisites are ready. Chronological entries below preserve historical checkpoints.
