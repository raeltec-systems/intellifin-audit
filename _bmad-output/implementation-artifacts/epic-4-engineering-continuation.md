...
- Browser/accessibility:161passed/1failed. Canonical P1 review/absence and credential-containment journeys pass. P4 passes its exact golden Gate, Observation/evaluation and declaration assertions, then incorrectly expects local snapshot reads in the browser-request ledger. This checkpoint corrects that test by verifying durable model selections against approved locators and actual registered artifact bytes; no golden assertion or runtime guard is weakened. Hosted rerun pending. The seven actual-worker/hydrated mutations remain running; do not cancel them with an early push.
...
- Next: retain the complete worker mutation results, push the corrected P4 test and verification reports, and complete every exact-candidate job; execute the live gate when runner settings and target prerequisites are ready. Chronological entries below preserve historical checkpoints.
