...
| 4.10 | One page Work Item, one Observation per distinct baseline key, real shared reconciliation/evaluation and exact failing Gate set | PG canonical P4 and both guard mutations pass. Browser exact Gate/Observations/evaluations/declaration assertions pass, then an incorrect browser-ledger assertion fails: snapshot reads reside in model turns, not fabricated HTTP requests. Correction pending |
| 4.11 | Dynamic golden attacks, hydrated closed answers, real worker SDK denial, catalog-discovered credential sink scans, overlapping browser-state isolation, workspace egress and terminal closure | All29 guard mutations pass, including seven actual-worker/hydrated mutations with32 selected baseline/removed-guard case pairs. Remote isolation blocked |
