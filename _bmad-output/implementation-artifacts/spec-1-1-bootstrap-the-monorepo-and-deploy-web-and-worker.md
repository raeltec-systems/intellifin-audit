...
- [x] `packages/infrastructure/src/config.ts` -- Zod schema for `DATABASE_URL`, `SERVICE_NAME` and telemetry; exported `loadConfig()` called only by composition roots -- AD-11. (Superseded 2026-09-02: `SCHEMA_RANGE_MIN`/`SCHEMA_RANGE_MAX` were removed from configuration after they drifted on the Story 1.2 release; the range is now the build constants `SUPPORTED_SCHEMA_MIN`/`SUPPORTED_SCHEMA_MAX` in `db/compat.ts`.)
