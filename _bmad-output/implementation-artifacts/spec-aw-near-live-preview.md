...
integration_baseline_commit: 86f35281c86fc15fdb94827e9be213052ed19f2d
context:
...
- [x] Implement the isolated process-local privacy/action coordinator and deterministic race tests; integration completed.
- [x] Add privacy/action coordinator and same-Page bounded capture port.
- [x] Add metadata runtime/epoch fencing, private broker and protected proxy delivery (migration 61 generated and applied).
...
- [ ] Complete composed separate-worker browser acceptance, including actual process loss; adapter-level privacy, authority, multi-viewer and decode cases already passed.
- [x] Record measured synthetic cadence/latency and capability limitations in P2/report.
...
- Client fetches are same-origin/no-store, bounded, and sequential. Each image is
  ...
  epoch before display. The status sample must be the same sequence/time or a
  strictly newer sequence/time in that public epoch. A newer safe capture never
  extends the decoded frame's original three-second age limit. Hide, stale age,
...
Integrated on the manager-transfer branch after d74ff29. Migration 61 was generated from the schema, with the preview guard appended and the exact schema inventory updated. Preserve all approved D3 tables/exports and the native POST rule. The 600 ms sampler, 500 ms admission floor and 500 ms sequential client polling provide measured headroom without weakening the one-FPS/two-second acceptance thresholds. See [the preview report](report-aw-near-live-preview.md) for actual passing and failed invocations, scope limits and benchmark artifacts. Independent review and final combined checks follow.

## Current implementation handoff — composed worker acceptance

The existing production preview implementation and ten adapter/browser tests are integrated in the root workspace. This handoff completes the missing proof; do not rebuild completed components or implement any other slice. Add a focused `tests/e2e/workspace-preview-worker.spec.ts` and test-local preload/helper under `tests/fixtures/`, reusing the actual compiled-worker spawn, synthetic provider, storage and frozen procedure setup patterns in `agent-evaluation-journey.spec.ts` and `prodconsole-agent-journey.spec.ts`. Use real application Run start, commands, durable transitions and worker-owned browser. Test fixtures may gate actual I/O with IPC; never fabricate domain execution facts or privacy outcomes.

Given two independently authenticated viewers and an actual worker-owned synthetic workspace, prove fresh same-workspace frames, safe Pause and Resume through application controls, a saved-authentication private interval with no newly delivered pixels and verified safe handback, access/disconnect clearing, and terminal Stop cleanup. Prove real worker SIGKILL leads to unavailable preview and cannot revive the old runtime; a separate second Run is acceptable for destructive worker-loss coverage. Record actual worker PID/runtime/Run linkage and metadata-only results without credentials, cookies, frame bytes or provider output. If an existing safe-pause restart intentionally replaces the workspace, assert the changed revision and refusal of old frames rather than claim continuity across replacement. P6 human private input is outside this slice.

Use existing supported frozen authentication flow; do not create new product auth authority to satisfy the test. Keep synthetic provider fixtures explicit and external provider acceptance unclaimed. Add CI execution of both preview suites with explicit synthetic-local configuration, using an isolated database/job if fixed-port or worker queues would conflict. Read existing workflow before changes.

Implementation ownership for this resumed handoff is the new worker proof/helper and required CI wiring, plus this spec's task status. Parent coordinates services, builds, database checks, independent review and publication; do not start services or heavy test/build commands concurrently without coordinating. Return concrete run commands and outstanding proof. All other production and existing preview tests are stable unless the worker proof exposes a defect: report that defect to parent before changing those shared files.

### Reattachment correction from composed acceptance

The actual worker Pause/Resume journey exposed that `run_workspace.revision` is a
mutable provisioning CAS: a healthy reattachment advances it while retaining the
same Page. Preview now stores the local workspace ID internally and holds its first
claim revision as the external generation. Publication/current-owner checks permit
an unchanged owner during PROVISIONING; viewer delivery remains OPEN-only. A new
workspace ID requires a higher generation, while the same ID can never reclaim an
expired runtime. Migration 61 was regenerated before publication; fresh v2 databases
and the 22-case preview/schema check passed. Fenced Pages refuse attachment.

The client retains a fresh displayed frame while it decodes and reauthorizes the
replacement, but clears displayed and pending URLs on refusal, privacy, expiry or
hiding. Full composed acceptance and independent review remain required.
