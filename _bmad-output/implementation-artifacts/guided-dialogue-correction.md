...
### Chat checkpoint and focused review corrections

Pushed `8435101cbe4ed6e629ef7aac18ddcfaa30e07f2e` (tree `d23fd283e869a8c363c819763ec19f8b0e131067`; exact tree of local tested `9693353`). CI 34683308137 passed typechecking, boundaries and all 4,151 unit tests. The browser run passed the fresh-Template chat and owner journeys and retained actual desktop/mobile chat captures, which were inspected. Its new reading-position check captured an intermediate browser keyboard-scroll position (88 pixels) before scrolling settled at zero; the assertion now waits for the requested scroll to settle before testing that subsequent reply updates preserve it. The serial remainder and full browser gate still require the next checkpoint.

The follow-up also keeps the latest message visible across viewport reflow, while preserving intentional reading position. It closes a dismissed uncertain request and retains completed rejected proposals without empty headings. Review identified two server corrections: reject foreign Origin before authorization can create a denial audit event, and filter configured credential references plus recognizable credential material in both partial and completed output. References stay server-side and never join model context. The SDK adapter withholds the unfinished trailing token so a credential split between chunks cannot be displayed before the output filter recognizes it. All 109 focused state, transport, application and SDK checks pass locally, including a split-token fixture; final hosted evidence remains pending in the PR.

The next pushed checkpoint is `5ff80d6599e8de5767d6bad026d660190ba04651`, tree `6d3d690f94b84de21d30d36f0aa6cd629c02edf2` (identical to local tested `dbbfef3`). During its hosted run, SDK source review exposed that valid structured JSON can resolve even after a missing completion event, provider failure or token cutoff. Three reproducing adapter tests failed against that behavior. The adapter now also requires a successful `stop` finish reason before returning a proposal; partial text and schema validity alone cannot authorize a completed receipt. These are transport guarantees, not claims about semantic faithfulness. Final pushed SHA and hosted results remain in the PR handoff.

CI 34683758314 passed the focused guided-authoring/owner browser step on `5ff80d6`, including streaming, reading position and mobile reflow. Local verification of the completion correction passes all 112 focused checks and pinned typechecking. The final push re-runs the required complete hosted gates; their exact results and screenshots are identified in PR #32.
