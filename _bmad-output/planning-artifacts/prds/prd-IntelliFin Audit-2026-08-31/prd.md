...
**Compiler-1 Run path baseline `[COMPILER-1 PATH]`.** The revision-2 scope below is built and stays in scope for recurring checks executed as Runs. The sixteen P0 capabilities the revision-2 directive named (logged in `.memlog.md`, 2026-09-01) remain this path's scope (FR-4–FR-44, NFR-5); the table that mapped them is retired.
