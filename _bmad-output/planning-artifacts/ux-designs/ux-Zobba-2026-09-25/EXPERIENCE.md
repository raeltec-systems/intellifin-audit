...
created: 2026-09-25
updated: 2026-09-25
amended: "2026-09-25 — the owner's decisions on the 18.2 screen review (design-acceptance register, Part D items 3, 5 and 6): Stop while a task waits, the Engagements empty sentence, the Settings sections"
...
| Settings (RS 15) | Settings | Profile · Connections · Methodology and skills · Permission defaults · Organisation details · Administration, each shown and editable according to the approved capabilities; a secondary 220px column with the main app on the rail. Connections opens the same connection-management surface the sidebar reaches, never a second configuration store. Admin manages methodology configuration and the administrator-controlled defaults; engagement-specific Permissions stay on the engagement, and a changed default never silently alters an active task or an approved check. A function not yet implemented stays unavailable. There is no Notifications section in this release (§13 item 2). Owner decision, 2026-09-25 (18.2 review, item 6). |
...
**Guidance versus Stop (R1.4, P4, P5; 4b §5 row 1).** Guidance typed while a step runs is acknowledged "Guidance queued for the next step" and, at the next step boundary, "Applied your guidance · …". Guidance that conflicts with completed work is named in the conversation with a question whether to redo it. Stop is a separate control, always reachable. Pressing Stop shows the transient **Stop requested** state (working mark, "Stopping after the current step…") until cessation is recorded; only then does the execution chip read **Stopped by you** and Zobba say what was kept. A completed external effect is never described as undone. **Stop while a task waits** (owner decision, 2026-09-25): Stop stays available on a nonterminal waiting task, subject to the person's authority. Stopping it cancels the task's continuation without treating the outstanding question or confirmation as answered: the wait is withdrawn, the work and the decision history are kept, and a later answer is refused and cannot revive the cancelled task. **Stop requested** shows until cessation is recorded; when no operation is in flight, cessation can be recorded promptly, and "Stopping after the current step…" is never shown when no step is executing. An external action already dispatched keeps its reconciliation path: stopping neither undoes it nor resolves an unknown outcome. Stop applies to the selected task, never to unrelated work.
...
| Workspace | Stop pressed while a step is executing, cessation not yet recorded | Working mark, "Stopping after the current step…"; execution chip Stop requested. |
| Workspace | Stop pressed while the task waits, no operation in flight | Execution chip Stop requested until cessation is recorded, which can be prompt; "Stopping after the current step…" is never shown. The outstanding question or confirmation is withdrawn, never treated as answered; a later answer is refused. The sentence is `[TO DESIGN]` with register row B17 (§3). |
| Workspace | stopped | Execution chip Stopped by you; Zobba says what was kept ("Stopped. Work so far is saved: …"). An external action already dispatched keeps its reconciliation path; it is never described as undone or resolved by the stop. |
...
2. **OS and push notifications** — deferred with the notification-policy contract; the template icon assets are filed for then. Notifications are in-app in this release, and Settings has no Notifications section in this release (owner decision, 2026-09-25); the in-app notification panel and the complete attention view stay required.
...
7. **Empty-state wording for Engagements** — decided at the 18.2 review (owner, 2026-09-25): "No engagements yet. Start a conversation to begin." DESIGN.md is aligned.

Decided and no longer open: item 7 (Engagements empty sentence, 2026-09-25), Q3 provenance footer, Q4 dark mode, Q5 icons, Q7 panel protection, Q8 promotion eligibility, Q12 templates, Q13 models and defaults, Q14 approval step (no Partner or Approver role), Q15 cost visibility (§14).
...
| 18.2 screen review | 2026-09-25 | Owner: the documentary review of the 23 screens approved with its findings; six register rows added (B17–B22); design gates follow the surface-part rule; Stop stays available on a nonterminal waiting task (§3); RS 17 and RS 18 are representative narrow references, not complete responsive acceptance; the Engagements empty sentence confirmed; Settings sections reconciled (§2), with no Notifications section in this release. No register row closed; no implementation authorised. |
