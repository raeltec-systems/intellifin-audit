...
| Any | Page could not be loaded (route boundary) | Heading "This page could not be loaded". Destructive Banner "This page could not be loaded. Check the Run's current state before repeating your last action." on a Run page, and "This page could not be loaded. Check the current state before repeating your last action." on any other page, with "Reload this page to read it again. Reloading from here does not repeat your last action. If the page keeps failing, tell a PoC Administrator." One control, "Reload this page": a plain link to the page's own address, query included, that reads the page again and never repeats the action. The boundary is reached by a page that could not be built and by a Server Action that committed and whose acknowledgement was lost, and it cannot tell the two apart, so it never says whether anything changed. The "Action failed" row stays for an action the platform refused. |
