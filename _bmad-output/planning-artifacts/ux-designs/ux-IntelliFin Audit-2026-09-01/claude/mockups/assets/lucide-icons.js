/* IntelliFin — self-hosted icon set.
   Icon glyph paths are from Lucide (https://lucide.dev), ISC licensed.
   This file provides a `window.lucide.createIcons()` shim compatible with the
   <i data-lucide="name"> markup the design-system Icon component emits, so the
   project needs no runtime CDN (production VPS deployments may have no CDN access). */
(function () {
  var I = {
  ...
  function buildSvg(name, attrs) {
    var inner = I[name];
    if (inner === undefined) inner = I["alert-circle"]; // visible fallback for unknown names
    ...
    svg.setAttribute("width", (attrs && attrs.width) || 24);
    svg.setAttribute("height", (attrs && attrs.height) || 24);
    svg.setAttribute("stroke-width", (attrs && attrs["stroke-width"]) || 2);
  ...
  function createIcons(options) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var svg = buildSvg(name, attrs);
      // preserve any classes the author put on the placeholder
