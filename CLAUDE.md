...
- **Every package and the root pin `typescript@7.0.2`.** dependency-cruiser 18.2.0 has no TypeScript 7 API yet, so `pnpm boundaries` prints a `missing-typescript-transpiler` warning and parses with its own fallback parser; verified 2026-09-02 that it still cruises every module, catches type-only vendor imports, and reads `.tsx`. Do not pin TypeScript 6 anywhere to silence the warning.
