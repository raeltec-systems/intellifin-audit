...
The owner's chat correction (2026-09-12) requires a scrolling message thread, a bottom composer and actual provider streaming. Display-only `AuthoringProgress` cannot enter acceptance; only a validated completed receipt can. The custom JSON POST route checks Origin against configured auth origin (never Host), authenticates before reading bounded input, and invokes the existing reserved-request use case. Disconnect stops delivery while bounded provider work finishes its receipt; retry the identical request. Keep the SDK Responses SSE fixture explicit, test incomplete output before completion, and never describe it as live-model evidence. See the chat addendum in the dialogue correction report.

AI SDK structured output validates JSON shape without requiring successful provider completion. The authoring adapter must also require `finishReason === 'stop'`: fixtures with valid JSON followed by EOF, `response.failed`, or a token cutoff otherwise resolve as proposals. Streamed tokens remain display-only until both completion and final validation pass. Reject foreign Origin before authorization can append a denial audit event; filter configured credential references in completed output as well as partials, and hold unfinished trailing tokens until they can be checked.
...
## 2026-09-09 — `sql.json` throws on any client `createDb` has wrapped

drizzle-orm's postgres-js driver rewrites the WRAPPED client's serializers for `json` (114) and `jsonb` (3802) to the identity, so on a postgres.js client that `createDb(sql)` has seen — every integration file's `sql` — `sql.json(obj)` hands the raw object to the wire encoder and throws `The "string" argument must be of type string…` before PostgreSQL sees a statement. On an unwrapped client the same call works and `${JSON.stringify(x)}::jsonb` double-encodes instead (the PR 21 note). `${JSON.stringify(x)}::text::jsonb` is right on both: a text parameter meets no JSON serializer and the server does the cast. Two things it hid, both repaired: `immutable-versions.test.ts` restored the `@current` pointer through `sql.json` and failed on any database that already held a published configuration, fresh or upgraded (CI never met it; the populated-upgrade proof did), and `procedures.test.ts` asserted a bare `rejects.toThrow()` on three raw-SQL Period cases, which the driver's TypeError satisfied before the CHECK was reached. A refusal that must be PostgreSQL's is asserted by SQLSTATE (`22008`, `23514`), never by a bare throw.

## 2026-09-09 — A guarded line's mutation anchors move with it

`scripts/verify-*-mutations.mjs` anchor each mutation on an EXACT source line (`before:`), and the hosted mutation gates stop with `Mutation anchor drift` before proving anything when that line changes. The C2 policy edit widened the worker's audited-retry guard and three anchors went stale for two pushes. When editing a line a harness anchors on, repoint the anchors in the same commit, keep the mutation and its killing test unchanged, check every `before:` occurs exactly once, and rerun the harness in a disposable detached worktree of the committed HEAD (it refuses a dirty tree).

## 2026-09-08 — D3: the 24-hour window's evidence path is explicit, frozen and proven

A compiler-1 time field the bound source stores under another name is supplied through an OPTIONAL per-condition `mapping` (time fields only, one column per field across the version, omitted when absent so legacy rows recompile byte for byte). `observationRuleValues` applies it after same-named columns and under grounded attributes; a column the source lacks is `missing or invalid Observation field termination_time`, and a date is never promoted to an instant. P-1's `disabled_time` is a VARIANT attribute (`variantAttributeLabels`, label `Disabled time` on LoanCore's page and in the catalogue) captured only when the version's Evidence Requirements name it, so default P-1 Runs are unchanged. E-000105 is the §D boundary case: exactly 24 hours is Compliant under the inclusive boundary. Fixtures: `canonicalLoanCoreCompliance({ disablementWindow: true })`, `startCanonicalLeaverSource(id, { terminationTime: true })`; journey: `tests/e2e/disablement-window-journey.spec.ts`. Contract: `docs/contracts/deterministic-evaluation-v1.md`. Not a universal mapping platform.
...
<!-- Story4.7 durable waits: release creates the waits queue; answers retain its original deadline job, whose closed-row delivery is superseded. Closed waits are immutable and SQL closure checks explicitly reject NULL discriminants. All producers reuse withRunExecutionContext for Observation, Evidence, Gate and Result writes. -->
<!-- Story4.6 gateway: real provider usage survives invalid/late responses and fallback; unaccounted attempts are explicit so the Run ledger retains a conservative reservation. Agent model code is a worker-only subpath, checked for source and built imports. -->
<!-- Story4.4 browser capture: logical search performs the real approved GET form with exact accessible labels. Capture requires an explicit CredentialGuard, refuses password/live secret surfaces, and binds semantic snapshot/PNG artifacts to the final sanitized location. It does not itself register an Observation. -->
<!-- Story4.4 storage checkpoint: generation33 admits structural-snapshot and screenshot Evidence kinds with distinct deterministic reservation keys. Existing artifact keys/digests and sealing guards are unchanged; capture producer wiring follows separately. -->
...
- **The read-only rule is applied ONCE, above routing, in `apps/northstar/src/server.ts`.** It refuses any operation a route has not DECLARED non-mutating, answering 405 with the route's own `Allow` and a body naming FR-3 — including on a path no route serves, because "there is nothing here" and "this system does not accept that" are different statements to a Run. It read "any method but `GET`/`HEAD`" until 2026-09-06, when the owner corrected the predicate: see "Read-only means no mutation of audited business data" at the bottom of this file. `read-only.test.ts` walks the exported `ROUTES` table and asserts BOTH directions, so a route added later is covered the moment it exists; every `Route` must declare a `probe` path, and `routes.test.ts` proves each probe matches its own pattern and is not shadowed by an earlier route.
...
- `[EXTENDED 2026-09-09]` On a client `createDb` has wrapped the opposite happens — `sql.json` throws and the JSON text passes through — so `${JSON.stringify(x)}::text::jsonb` is the one spelling right on both; see the 2026-09-09 `sql.json` note at the top. **`${JSON.stringify(x)}::jsonb` double-encodes in postgres.js.** The cast tells the driver the parameter is jsonb and the driver stringifies the string again, so the column holds a JSON string. Use `sql.json(x)` or pure SQL (`jsonb_set`, `to_jsonb`). A "malformed value is refused" test passes either way, which is how it hid.
...
- **The credential is presented inside the DESTINATION's own frozen origin, and nowhere else.** `[SUPERSEDED, mechanism only]` It was attached by the request INTERCEPTION rather than by `setExtraHTTPHeaders`, because the header would otherwise ride every request the page makes until it is cleared — and the workspace's egress allowlist is the UNION of every web Target's frozen origins, so the moment a plan names two web systems the first one's credential would reach the second. The sign-in now types into the system's own form, so that injection became unreachable and was removed rather than left as a branch nothing exercises; the GUARANTEE is unchanged and is now the form-action origin check in `signIn`. Proven by mutation, then as now: removing the origin check lets a cross-origin form receive the credential.
...
### Owner review: a failed release must not lose what it could not give back (2026-09-06)

- **A swallowed failure that is followed by a REPLACEMENT destroys the only thing that could ever undo it.** `provisionWorkspace`'s reattach branch released the stale identity with `.catch(() => undefined)` and then fell through to `create`, overwriting `checkpoint.workspaceId`. The adapter already resolves `InvalidSessionId` and a 404 as SUCCESS — on a release, "the stored identity is gone" is the outcome asked for — so a release that THROWS is an outage, a capacity refusal or a policy denial, and the remote session is very possibly still running. `run_workspace` is the only durable record of it and the reaper reads that row, so after the replacement nothing anywhere could name the old session: it would be held until the provider's own plan deadline with no operator able to find it. **The general shape: `.catch(() => undefined)` is safe only where the next statement does not depend on the call having worked.** Here the next statement was the one that made it unrecoverable.
- **So the release failure ENDS the attempt instead, and keeps the identity.** A private `StaleReleaseFailed` — thrown, never returned, and carrying no `cause`, because the diagnostic vocabulary is closed and an error message is where an endpoint or a value rides into an immutable event — reaches the file's one failure handler, which writes `workspace-release-failed` and leaves `workspaceId` and `expiresAt` untouched. It is RETRYABLE and deliberately not a `WorkspaceFailureCode`: `TERMINAL_CODES` answers "will CREATING refuse identically again?", which has no bearing on a release, and a retry costs one reattach and one release against a session that may still be alive. The budget bounds it; when it is spent the row is `FAILED` and STILL names the workspace.
- **An EXPIRED identity is the one branch where swallowing is right, and the two must not be collapsed.** Past the provider's HARD deadline the session auto-released itself, so there is nothing to leak, the call is a courtesy, and a failure means it was already gone — the outcome asked for. A replacement is correct there whatever the release answered. The mutation that collapses the two is killed only by the companion test: the pre-existing expired test releases successfully, so it cannot see the difference.
- **Keeping the identity is worth nothing if the backstop cannot see the row it is kept on.** `reapableRunIds` selected `PROVISIONING`, `OPEN` and `RETRY` — and `FAILED` is exactly where a row ends when the budget is spent, which is where every path that preserves an identity through a failure lands. The one row the reaper most had to find was the one row it could never select, so the sweep stopped precisely where the leak started. **A read that filters on a state vocabulary has to be re-read whenever a new way of reaching one of those states is added**; the fix and the read that consumes it are one change, not two.
- **`remaining()` is evaluated ONCE and outside both branches.** A lease already spent is not a release that failed, and recording it as one would name something that never happened: it throws `unavailable`, which is honest — nothing was released and nothing was created — and the identity survives either way.
- **The fake's `release` records the ref BEFORE it can throw**, so a test can tell "attempted and failed" from "never attempted". A fake that threw first would let a build that skipped the release entirely pass the regression test.

### Read-only means no mutation of audited business data (2026-09-06)

The owner overturned the rule Story 1.8 wrote and Story 4.2 built around: *read-only means
no mutation of audited business data; it should not force a specially invented GET-only
sign-in solely to satisfy an earlier fixture rule.* The decision, the error it corrects and
what is deliberately NOT changed are in
`_bmad-output/implementation-artifacts/epic-4-loancore-authentication-decision.md`.

- **The system-level guard refuses MUTATION, not methods, and every property that made the
  method rule good survives.** One rule, applied ONCE, ABOVE routing in `handleRequest`;
  fail-closed; a write to a path no route serves still refused rather than 404'd; the
  denial still JSON on every surface and still naming FR-3. Only the predicate changed:
  from "is the method GET or HEAD?" to "is this a declared non-mutating operation?". The
  HTTP method was being used as a PROXY for mutation, and a sign-in POST creates a session
  and mutates nothing FR-3 constrains — FR-3 constrains what the platform may INVOKE, which
  is enforced against the registration's frozen `permitted_actions` by `authorizeToolAction`.
- **The declaration is the type's default, not a convention.** `Route.nonMutating` is
  OPTIONAL and its absence means the route mutates, so a route added by a later story is
  refused every method — GET included — until it says otherwise. `read-only.test.ts` walks
  `ROUTES` and asserts BOTH directions per route, because a suite that asserted only
  refusals would pass against a guard that refuses everything, sign-in included. It also
  drives a hand-made undeclared route, which is the fail-closed branch no real route takes.
  `routes.test.ts` requires every route to declare, and to declare in upper case: a
  declaration spelled `post` permits nothing and reads as though it permitted something.
- **`Allow` is per route now.** The matched route's declaration, or `GET, HEAD` for a path
  no route serves — because a read there gets a real answer, this system's own 404. A
  matched route that declares nothing gets an EMPTY `Allow`: saying `GET, HEAD` while the
  guard refuses a GET would be a header contradicting the response beside it.
- **`READ_METHODS` was renamed `NON_MUTATING_READS`.** The name said "the whole rule" and
  the value is now a declaration a route reaches for. `isReadMethod` is gone with it.
- **LoanCore has a real sign-in form, and `/loancore` IS the sign-in surface.** A caller
  with no session is shown the form at the frozen allowed origin itself, so the platform
  navigates to the origin its Procedure Version froze and needs to guess no path and parse
  no page to find the form. `/loancore` and `/loancore/sign-in` are the two public paths,
  allowlisted the way `apps/web`'s `isPublicPath` allowlists `/sign-in`; every other
  `/loancore` path answers 401. The `WWW-Authenticate` challenge names the FORM
  (`FormSignIn realm="LoanCore", form="/loancore/sign-in"`) rather than `Bearer`, which
  would name a way of authenticating this system no longer has.
- **The header sign-in was REMOVED, not kept beside the form.** Leaving one would make the
  form decorative: every test would pass with the form deleted, which is the failure
  "assert both directions" exists to prevent.
- **The workspace signs in through the form, and the form is judged before anything is
  typed.** Exactly one `<form>` with exactly one `<input type="password">` and exactly one
  submit control; the form must declare `POST`; its resolved action AND the submitter's own
  `formaction` must be inside the destination's frozen origin. A stated mechanism contract,
  not a heuristic — a mechanism that picked one of several forms would be choosing, on a
  page a Target System controls, where a credential goes. That origin check is what the
  interception's header injection used to do, which is why the injection could be deleted
  rather than left unreachable.
- **`ResolvedCredential` gained `enter(field)` and still has no field holding a value.**
  `authorize` writes `Authorization: Bearer <token>`, which is a PRESENTATION; a form field
  carries the credential itself, so they are two methods rather than one with a mode. The
  sink is nameless: which field a credential goes in is the mechanism's business, and a
  resolver that chose the name would be one that knows what a sign-in form looks like.
- **`BrowserActionResult.method` is read back, and generation 31 widens
  `run_tool_action_method` to `TOOL_ACTION_METHODS` (`GET`, `HEAD`, `POST`).** The method
  recorded is the one that produced the action's FINAL response, walked back through the
  redirect chain the system chose — so a sign-in records the form's `POST` and not the
  `GET` the browser was left on after the 303. Recording only the `GET` would put the
  method-as-a-proxy-for-mutation equivalence back one layer down, in the log a reader checks
  the guarantee against. The set stays CLOSED, and `performToolAction` refuses a method
  outside it as a `contract` failure rather than letting a row violate the CHECK and take
  the transaction with it.
- **Both credential-entry cases now exist in the fixtures, and both are tested.** The typed
  form is LoanCore's; the header echo is `/accessgate/credential-echo`, on the adapter path,
  untouched. The integration suite gained a system whose sign-in form posts to ANOTHER
  frozen origin — both inside the workspace's egress allowlist, so the interception would
  let it through — and asserts `sign-in-scope-violation` plus a server log showing nothing
  was posted to either origin.
...
### Verifying another agent's continuation (2026-09-08)

Codex pushed 128 commits to `codex/epic-4-agent-runs` while this session was rate-limited. The
method that checked them is written down so the next handover is checked the same way rather
than trusted, and the two harness mistakes it produced are recorded beside it.

- **Read the authoritative signal, never the report.** CI verdicts were read at the JOB level
  (five jobs, each `success`), the live-provider claim was read from the CI job LOG (the
  worker's five model turns, the `AWAITING_AUDITOR` state and the release timestamp are all
  in it), and the Railway deployment from the Railway API. The engineering report agreed
  with all three; it was not the evidence.
- **Both migration paths, then parity.** Fresh install empty → 41 and upgrade 32 → 41 in two
  databases, then `information_schema` column, constraint and trigger counts compared
  (516 / 737 / 40 on both). A migration chain that installs is not proven to upgrade.
- **Scan for weakened tests, not only for failing ones.** Deleted test files, added
  `.skip`/`.only`/`.fixme`, and `expect(` lines REMOVED from pre-existing test files (15,
  each read, each replaced by a stronger one). A green suite says nothing about what it
  stopped asserting.
- **Grep the single-writer seams by hand** (`run_observation` inserts, `run_result` writes,
  `audit_run.state` writers, `browser.perform` call sites) and have a read-only mapping agent
  do it independently. Two answers that agree from different methods.
- **Check what the other agent changed in YOUR files first.** A ten-line patch to the
  generation-32 migration and a lost-claim release of an ATTACHED workspace handle were both
  defects in this session's own work, found by Codex's populated-upgrade and race tests.
- **`nvm use` does not take effect inside `nohup bash script`.** The gate started under Node
  22.22.2 against a repo pinned to 24.20.0, and only the interpreter line written into the
  log exposed it. Printing the version is not enforcing it: the script now exports
  `/opt/node24/bin` explicitly and REFUSES to run on any other Node or pnpm version.
- **`pkill -f <pattern>` matched this shell's own command line, a second time.** Exit 144,
  and everything after the `pkill` in that command never ran, so the wrong-Node gate kept
  running while the shell that meant to kill it was dead. Kill by PID, or spell the pattern
  `"[g]ates-codex"` so the regex cannot match its own literal text.
- **Three decision-log notes went stale without a marker.** A change that supersedes a rule
  written here must mark the old entry in the same commit, or the next agent implements the
  old rule. `[EXTENDED]` and `[SUPERSEDED]` markers with a pointer are the convention.
