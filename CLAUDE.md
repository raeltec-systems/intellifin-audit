...
- **The focus ring is declared once, in `globals.css`, and components have no stylesheet to suppress it in.** It is `#0F766E` everywhere; inside the navy sidebar a white 2px band at the element edge gives the ring a light ground to sit against, because teal-700 on navy is 2.7:1. For the same contrast reason the active nav marker on that one dark surface is `--color-teal-500`, not `--color-teal`.
