...
describe.skipIf(!url)('preview durable owner, privacy and viewer fences', () => {
  ...
  it('keeps an unchanged Page owner through reattachment without reviving an expired runtime', async () => {
    const f = await fixture();
    await client`UPDATE run_workspace SET revision=revision+1,status='PROVISIONING' WHERE run_id=${f.ref.runId}`;
    // The already-owned Page may remain alive, but no viewer delivery while attaching.
    expect(await f.store.current(f.metadata)).toBe(true);
    expect(await f.store.publish(f.metadata)).toBe(true);
    expect(await f.store.authorized(f.ref.runId, viewer)).toBeNull();
    await client`UPDATE run_workspace SET revision=revision+1,status='OPEN' WHERE run_id=${f.ref.runId}`;
    expect(await f.store.authorized(f.ref.runId, viewer)).toMatchObject({ workspaceRevision: 1, runtimeId: f.metadata.runtimeId });
    expect(await f.store.claim(f.ref, randomUUID())).toBeNull();
    await expect(client`UPDATE run_workspace_preview SET workspace_revision=3 WHERE run_id=${f.ref.runId}`).rejects.toMatchObject({ code: '23514' });
    await client`UPDATE run_workspace_preview SET expires_at=clock_timestamp()-interval '1 second' WHERE run_id=${f.ref.runId}`;
    expect(await f.store.current(f.metadata)).toBe(false);
    expect(await f.store.publish(f.metadata)).toBe(false);
    expect(await f.store.claim(f.ref, randomUUID())).toBeNull();
  });
  it('replacing a live workspace immediately fences its unexpired owner', async () => {
    const f = await fixture();
    const replacement = { ...f.ref, workspaceId: ids.next() };
    await client`UPDATE run_workspace SET revision=2,workspace_id=${replacement.workspaceId} WHERE run_id=${f.ref.runId}`;
    expect(await f.store.current(f.metadata)).toBe(false);
    expect(await f.store.publish(f.metadata)).toBe(false);
    expect(await f.store.authorized(f.ref.runId, viewer)).toBeNull();
    const runtimeId = randomUUID();
    expect(await f.store.claim(replacement, runtimeId)).toBe(2);
    expect(await f.store.publish({ ...f.metadata, workspaceRevision: 2, runtimeId })).toBe(true);
    expect(await f.store.authorized(f.ref.runId, viewer)).toMatchObject({ workspaceRevision: 2, runtimeId });
    expect(await f.store.current(f.metadata)).toBe(false);
    expect(await f.store.claim(f.ref, randomUUID())).toBeNull();
  });
