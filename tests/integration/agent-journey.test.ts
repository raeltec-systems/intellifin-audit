...
describe.skipIf(!url)('local browser agent journeys through PostgreSQL registration', () => {
  ...
  async function seed(employeeId: string, name: string) {
    ...
    const registration = { registrationId: ids.next(), displayName: 'LoanCore', kind: 'web' as const, allowedOrigins: [origin], applicationIdentity: '', credentialRef: REF, permittedActions: ['navigate', 'search', 'read-attribute'] as const, attributeLabelPatterns: ['Employee ID', 'Full name', 'Status', 'Username', 'Roles'], secondaryKey: 'Full name' };
