...
describe.skipIf(!databaseUrl)('worker heartbeat against a migrated PostgreSQL 18', () => {
  ...
  afterAll(async () => {
    // beforeAll can fail before `sql` is assigned; tearing down must not then throw
    // a second, more confusing error over the first one.
    if (!sql) return;
  ...
  it('upserts exactly one row per hostname and refreshes seen_at', async () => {
    ...
    const rows = await (sql as Sql)<{ hostname: string; seen_at: Date | string }[]>`
