...
schema = dataset['declared_schema']
...
cover.update(source='canonical-single-leaver', covers='single-leaver.csv', generation=dataset['generation'] + '.single-' + sys.argv[1],
             row_count=len(selected), content_digest={'algorithm': 'sha256', 'value': hashlib.sha256(raw.encode('utf-8')).hexdigest()})
