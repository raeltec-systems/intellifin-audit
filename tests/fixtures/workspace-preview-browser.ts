  ...
  try {
    ...
    return { sql, actorId, runId, ref, runtimeId, store, execution, page, context, viewport, pageEvents, viewportChanges,
      ...
      async signIn() {
        ...
        // Authentication is scoped to the frozen target base, which contains both
        // the sign-in endpoint and its authenticated account destination.
        return execution.perform(ref, { action: 'navigate', destination: origin, authenticationDestination: `${origin}/sign-in`, credential }, 10_000);
      },
      captureRegistered() {
