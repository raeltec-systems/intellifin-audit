...
test.describe('protected preview through compiled adapter, PostgreSQL, broker and web route', () => {
  ...
  test('two authenticated sessions see changing samples from one unchanged agent Page, with measured performance', async ({ browser, preview: f, viewer }, info) => {
    ...
    try {
      ...
      expect(Boolean(b.find(cookie => cookie.name.includes('session_token'))?.value) &&
        b.find(cookie => cookie.name.includes('session_token'))?.value !== a.find(cookie => cookie.name.includes('session_token'))?.value,
      ...
      expect(frames.length).toBeGreaterThan(5); expect(captureIntervals.every(value => value >= 490)).toBe(true);
  ...
  for (const kind of ['preview', 'registered'] as const) {
    test(`stored sign-in drains a real crossing ${kind} screenshot before credential entry`, async ({ preview: f, viewer }) => {
      ...
      const bytes = await test.step('The actual screenshot reaches the held completion', () => gate.arrived.promise);
      ...
      await test.step('Credential entry follows the drained screenshot', () => Promise.race([
        loginGate.arrived,
        signInResult.then(() => { throw new Error('Sign-in settled before reaching the synthetic credential endpoint'); }),
      ]));
  ...
  for (const transition of ['private', 'role', 'session', 'runtime', 'worker-loss'] as const) {
    test(`a real decoded image cannot enter the stage after ${transition} changes`, async ({ preview: f, viewer }) => {
      ...
      // A fresh displayed frame stays visible while the next real decode is held.
      // Polling must not blank the stage before it has a replacement or refusal.
      await expect(visibleFrame(viewer)).toBeVisible();
