...
test('a fresh Template leads through choices, a precise test-design reply, saved review and the actual compiled plan', async ({ page }, testInfo) => {
  ...
  const conversation = writing.getByRole('log', { name: 'Conversation with IntelliFin' });
  await expect(conversation.getByRole('article', { name: 'Message from you', exact: true })).toHaveCount(2);
  await expect(conversation.getByRole('article', { name: 'Message from you', exact: true }).last()).toContainText(correction);
  await expect(writing.getByText('Compare with saved wording', { exact: true })).toBeVisible();
  await expect(writing.locator('.ls-writing__comparison-disclosure')).not.toHaveAttribute('open');
  const geometry = await writing.evaluate(node => {
    const thread = node.querySelector('.ls-chat__thread')!.getBoundingClientRect();
    const composer = node.querySelector('.ls-chat__composer')!.getBoundingClientRect();
    return { threadBottom: thread.bottom, composerTop: composer.top };
  });
