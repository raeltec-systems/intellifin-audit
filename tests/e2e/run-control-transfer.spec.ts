...
test('explicit grant, review, reload and lost-response recovery update both authenticated viewers', async ({ browser }) => {
  ...
  try {
    ...
    const observerTransfer = auditor.getByRole('region', { name: 'Conversation history', exact: true })
      .getByRole('article').filter({ hasText: 'Took control using explicit manager permission.' });
    await expect(observerTransfer).toHaveCount(1);
    await expect(observerTransfer).toContainText(name);
    await expect(observerTransfer).not.toContainText('r'.repeat(1000));
    await expect(observerTransfer).not.toContainText('resumed');
