...
const controlNames = {
  p1: `E2E Filter needle P-1 ${stamp}`,
  p2: `E2E Filter haystack P-2 ${stamp}`,
};
...
async function draftFor(
  ...
  const inputs = { ...executablePlanInputs(), templateId, controlName };
...
test.beforeAll(async () => {
  ...
  // Two Drafts, deliberately different Templates and deliberately sharing no word except
  // "E2E Filter" and their Template ordinal, so a search for one is never a search that
  // also matches the other by accident.
  await draftFor(db, procedureIds.p1, versionIds.p1, 'P-1', controlNames.p1, auditorId);
  await draftFor(db, procedureIds.p2, versionIds.p2, 'P-2', controlNames.p2, auditorId);
});
...
test.describe('the Procedures list as an Auditor', () => {
  ...
  test('the search box narrows to a name and survives a reload of the URL it produced', async ({ page }) => {
    ...
    await page.getByLabel('Search by name or Template').fill('needle');
    ...
    await expect(page.getByLabel('Search by name or Template')).toHaveValue('needle');
  ...
  test('the status and owner filters combine with the search to narrow to one Procedure', async ({ page }) => {
    await page.goto('/procedures');
    await page.getByLabel('Newest version').selectOption('DRAFT');
    await page.getByRole('button', { name: 'Apply', exact: true }).click();
    await expect(page).toHaveURL(/[?&]state=DRAFT/);
    await expect(page.getByText(controlNames.p1)).toBeVisible();
    await expect(page.getByText(controlNames.p2)).toBeVisible();

    await page.getByLabel('Accountable for the newest version').selectOption(auditorId);
    await page.getByLabel('Search by name or Template').fill('needle');
    await page.getByRole('button', { name: 'Apply', exact: true }).click();
    await expect(page).toHaveURL(/[?&]owner=/);
    await expect(page).toHaveURL(/[?&]state=DRAFT/);
    // All three filters together, and only ONE of these two fixtures matches every one:
    // the state and owner both name P-2 as well, so only the search narrows to P-1 alone.
    await expect(page.getByText(controlNames.p1)).toBeVisible();
    await expect(page.getByText(controlNames.p2)).toHaveCount(0);
...
test.describe('the Procedures filter with no JavaScript', () => {
  ...
  test('is a native GET submission that mutates nothing', async ({ page }) => {
    await page.goto('/procedures');
    await page.getByLabel('Search by name or Template').fill('haystack');
    await page.getByRole('button', { name: 'Apply', exact: true }).click();
    await expect(page).toHaveURL(/[?&]search=haystack/);
    await expect(page.getByText(controlNames.p2)).toBeVisible();
