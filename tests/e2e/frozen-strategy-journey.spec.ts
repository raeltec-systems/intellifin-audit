...
async function startWorker(): Promise<void> {
  ...
  const worker = spawn(process.execPath, [
    '--import',
    pathToFileURL(resolve('tests/fixtures/frozen-strategy-worker-preload.mjs')).href,
    ...
    env: {
      ...
      MODEL_ID: '',
      MODEL_API_KEY: '',
      ANTHROPIC_API_KEY: 'synthetic-strategy-interception',
      STRATEGY_PROOF_VERSION_ID: versionId,
...
test.describe('frozen P-1 strategy through the actual compiled worker', () => {
  test.use({ storageState: AUTH_STATE.auditor, viewport: {width:1280,height:800} });
  test('reviews the exact fallback, commits one linked action, and preserves covered absence', async ({ page }) => {
    ...
    await expect.poll(async()=>(await sql`SELECT opportunity_id FROM run_strategy_cursor WHERE run_id=${runId}`)[0]?.opportunity_id,{timeout:90000}).toBeTruthy();
    await page.reload();
    await page.getByRole('button',{name:'Acquire control',exact:true}).click();
    await expect(page.getByText('Frozen strategy available: Full name search.',{exact:false})).toBeVisible();
    await page.locator('#run-conversation-message').fill('strategy: p1.full-name');
    await page.getByRole('button',{name:'Send message',exact:true}).click();
    const review=page.getByRole('button',{name:'Review Full name search',exact:true});
    await review.click();
    const dialog=page.getByRole('dialog',{name:'Use the frozen Full name search?',exact:true});
    await expect(dialog).toContainText('Queued does not mean applied');
    await expect(dialog.getByRole('link',{name:'Primary search evidence'})).toBeVisible();
    ...
    await review.click();
    await dialog.getByRole('button',{name:'Queue Full name search',exact:true}).click();
    await expect.poll(async()=>(await sql`SELECT t.state FROM run_strategy_transition t JOIN run_strategy_selection s ON s.command_id=t.command_id WHERE s.run_id=${runId} ORDER BY t.sequence DESC LIMIT 1`)[0]?.state,{timeout:60000}).toBe('applied');
    const selected=await sql`SELECT t.state,t.tool_action_id,a.action,a.parameters FROM run_strategy_transition t JOIN run_strategy_selection s ON s.command_id=t.command_id LEFT JOIN run_tool_action a ON a.tool_action_id=t.tool_action_id WHERE s.run_id=${runId} ORDER BY t.sequence`;
    expect(selected.map(row=>row.state)).toEqual(['interpreted','queued','dispatched','applied']);
    expect(selected[2]!.tool_action_id).toBe(selected[3]!.tool_action_id);
    expect(selected[3]).toMatchObject({action:'search',parameters:[{name:'name',value:source.row.full_name}]});
    try {
...
test.afterAll(async () => {
  const cleanup=await Promise.allSettled([stopWorker?.(),storage?.close(),source?.close()]);
  stopWorker = undefined;
  if (!sql) return;
  try {
    ...
    if (runIds.length > 0) {
      ...
      await sql`DELETE FROM run_strategy_cursor WHERE run_id = ANY(${runIds}::uuid[])`;
      await sql`DELETE FROM run_strategy_selection WHERE run_id = ANY(${runIds}::uuid[])`;
      await sql`DELETE FROM run_strategy_opportunity WHERE run_id = ANY(${runIds}::uuid[])`;
